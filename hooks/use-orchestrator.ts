...
User Message: "${userText}"`,
        config: {
          ...
          responseSchema: {
            ...
            required: ["messageType", "serviceType", "location", "urgency", "timeRaw", "isInappropriate"]
          }
        }
      });

    const intent: Intent & { isInappropriate: boolean } = JSON.parse(intentResponse.text || "{}");
    ...
    activeIntent.current = intent;
      ...
      updateLog(intentLogId, { 
        status: 'success', 
        details: JSON.stringify(intent, null, 2)
      });
...
Instructions:
1. Use natural, warm Roman Urdu or English - only one, resonating with the input content languge. it should be a bit professional yet slightly casual, not too much emojis.
